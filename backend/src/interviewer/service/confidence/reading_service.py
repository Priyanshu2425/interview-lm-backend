...
from ...model.readings_models import CoverageReading, MasteryReading, TopicReading
...
class SessionReadingService:
    ...
    def report_of(self, reading: SessionReading):
        ...
        from ...model.report_models import SessionReport
    ...
    def summary_of(self, reading: SessionReading):
        ...
        from .summary_service import SessionSummary
        ...
        per_topic = [
            {
                **topic_reading(
                    TopicReading.of(v["topic_id"], self._conf.get(f.candidate_id,
                                                             v["topic_id"])),
                    with_posterior=True,
                ),
        ...
        return SessionSummary(
            ...
            coverage=asdict(CoverageReading.of(all_readings,
                                     topics_total=len(self._corpus.topics))),
            mastery=asdict(MasteryReading.of(all_readings)),
            per_topic=per_topic,
            untested_modules=[u for u in untested if u["topics_untested"] > 0],
            spend=self._spend(reading),
        )
    ...
    def _reading(self, candidate_id: str, topic_id: str, row: dict) -> dict:
        ...
        """
        r = TopicReading.of(topic_id, self._conf.get(candidate_id, topic_id))
        return {
            **topic_reading(r),
    ...
    def _spend(self, reading: "SessionReading") -> dict:
        """What this Session cost, on the same terms `/spend` reports.

        Every Visit, not only the answered ones: the ledger charged for a Visit
        that was opened and abandoned, and leaving it out understated the total
        against the endpoint that includes it. Planning is charged too, and is
        carried on its own line rather than folded into the Visits.
        """
        from ...model.spend_models import SessionSpend, VisitCost

        f = reading.facts
        if self._credits is None or f.payment_route != "credits":
            # BYOK carries null, never 0 — zero reads as "it was free".
            return {"credits": None, "planning": None, "per_topic": None}
        spend = SessionSpend(
            route=f.payment_route,
            planning=self._credits.visit_cost(f"plan_{f.session_id}"),
            visits=tuple(
                VisitCost(
                    topic_visit_id=v["topic_visit_id"],
                    topic_id=v["topic_id"],
                    state=v["state"],
                    credits=self._credits.visit_cost(v["topic_visit_id"]),
                )
                for v in reading.visits
            ),
