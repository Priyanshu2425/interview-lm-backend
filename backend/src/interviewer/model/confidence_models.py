...
class Posterior:
    ...
    def updated(self, score: float, weight: float) -> "Posterior":
        """This posterior, plus one graded Topic Visit."""
        d = EvidenceDelta.of(score, weight)
        return Posterior(self.alpha + d.alpha_delta, self.beta + d.beta_delta)
...
class EvidenceDelta:
    ...
    @classmethod
    def of(cls, score: float, weight: float) -> "EvidenceDelta":
        """alpha += w*s and beta += w*(1-s).

        `s` and `w` measure different things and are never conflated. `w` is
        how far the Grading Mode is trusted; `s` is how good the answer was.
        Hint assistance lands in `s` — an answer reached after two hints is a
        real answer worth roughly half — and never as a reduced weight.
        """
        if not 0.0 <= score <= 1.0:
            raise ValueError(f"score must be in 0..1, got {score}")
        if weight not in (1.0, 0.7, 0.5):
            raise ValueError(
                f"weight must be a known Grading Mode weight, got {weight}"
            )
        return cls(weight * score, weight * (1.0 - score))
