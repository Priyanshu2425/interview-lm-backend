...
def test_modules_carry_real_topic_and_ground_truth_counts(client):
    rows = client.get("/v1/skills/modules").json()
    assert len(rows) == 15
...
def test_modules_can_be_filtered_by_track(client):
    dsa = client.get("/v1/skills/modules", params={"track": "dsa"}).json()
    assert len(dsa) == 7
...
def test_scope_reports_what_the_chosen_modules_can_produce(client):
    rows = client.get("/v1/skills/modules", params={"track": "aiml"}).json()
    ids = [rows[0]["module_id"], rows[5]["module_id"]]  # Data Foundations + NLP
    s = client.get("/v1/skills/scope", params=[("module_id", i) for i in ids]).json()
...
def test_an_empty_scope_reports_nothing_rather_than_failing(client):
    ...
    assert s == {
        ...
        "strongest_mode": None,
        # ISSUE-0040. Nothing to examine takes no time, and says so with a
        # zero rather than by leaving the field out.
        "suggested_seconds": 0,
        "minimum_seconds": 0,
...
def test_a_module_without_answer_keys_is_selectable_and_says_so(client):
    ...
    s = client.get(
        "/v1/skills/scope", params=[("module_id", genai["module_id"])]
    ).json()
...
def test_no_corpus_response_carries_a_difficulty_field(client):
    for path in ("/v1/skills/modules", "/v1/skills/tracks"):
...
def test_no_corpus_response_leaks_ground_truth_text(client):
    ...
    body = client.get("/v1/skills/topics/cmrovsvm21xy1qj0fmr2rinvz").json()
...
def test_an_unknown_topic_is_a_404(client):
    assert client.get("/v1/skills/topics/nope").status_code == 404


def test_provenance_says_which_extract_this_is(client):
    p = client.get("/v1/skills/provenance").json()
