...
from ..ending_service import EndReason, SessionEnding
from .machine_service import Deps, build_graph
...
class SessionRunner:
    ...
    def _park_on_provider_failure(self, session_id: str, e: BaseException):
        ...
        if failure is None:
            return None
        from ...model.failures_models import Cause, Route, UserFacingEvent
        ...
        event = UserFacingEvent.of(
            route=route, cause=cause, provider=failure.provider
        )
    ...
    def _as_provider_failure(e: BaseException):
        from ...service.metering.client_service import ProviderFailure
