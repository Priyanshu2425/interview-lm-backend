...
from .confidence_models import Posterior
...
class CoverageStanding:
    ...
    @classmethod
    def examined_counts(cls, posteriors: dict[str, list[Posterior]]) -> dict[str, int]:
        """Per Candidate, how many of these Topics read above the Evidence Floor.

        Counted here rather than in the repository, and never in SQL. The Floor is
        a property of the posterior's spread: a query approximating it with
        `alpha + beta > n` would be a second implementation of the rule, and a
        Python count inside a store would be this rule living somewhere a reader
        of `coverage_percentile` would never think to look.

        A Candidate with rows but none reportable counts zero rather than being
        absent — they have been examined, and `coverage_percentile` distinguishes
        the two.
        """
        return {
            candidate_id: sum(1 for p in rows if p.band.reportable)
            for candidate_id, rows in posteriors.items()
        }
