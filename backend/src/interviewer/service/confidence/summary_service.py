...
from ...model.corpus_models import Corpus
from ...model.confidence_models import Band
from .reading_service import topic_reading
from ...model.readings_models import CoverageReading, MasteryReading, TopicReading
from ...repository.core import ConfidenceStore
...
class CandidateReadings:
    ...
    def weakest(self, candidate_id: str, limit: int = 10) -> list[dict]:
        ...
        readings = [
            TopicReading.of(tid, p) for tid, p in self._conf.all_for(candidate_id).items()
        ]
    ...
    def candidate_readings(self, candidate_id: str) -> dict:
        ...
        readings = [TopicReading.of(tid, p) for tid, p in stored.items()]
        titles = {t.id: t.title for t in self._c.topics}
        return {
            "coverage": asdict(CoverageReading.of(readings, topics_total=len(self._c.topics))),
            "mastery": asdict(MasteryReading.of(readings)),
            "topics": [
                {
                    **topic_reading(r, with_posterior=True),
                    "title": titles.get(r.topic_id, r.topic_id),
