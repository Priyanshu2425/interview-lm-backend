...
from ...repository.core import ConfidenceStore, EvidenceLedger, VisitLifecycle
...
from ...service.ending_service import SessionEnding
from ...service.judge.judge_service import Judge
from ...service.judge.question_writer_service import QuestionWriter
...
def build_graph(d: Deps):
    ...
    def decide_next(state: SessionState) -> dict:
        ...
        if d.credits is not None and _route(state) == "credits":
            from ...model.credits_models import Balance

            balance = d.credits.balance(state["candidate_id"])
            if not Balance(balance).clears_headroom():
                overran = balance < 0
                return {
                    "finished": True,
